/*Funções e variáveis dos elementos da NavBar */

url = 'https://test-final.b8one.academy/menu'
...
const iconConfig = `
    <svg class="nav__icon"  width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <g clip-path="url(#clip0_343_3999)">
...
`
const arrow = `<svg class="nav__icon--arrow" data-arrow width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const dropSell =
`<div class="sell__action" data-dropdown>
                <ul class="sell__list">
                    <li class="sell__item"> <button>gerador de link</button></li>
                    <li class="sell__item"> <button>vitrine digital</button></li>
                    <li class="sell__item"> <button>carrinhos compartilhaveis</button></li>
                    <li class="sell__item"> <button>carinhos abandonados</button></li>
                </ul>
            </div>`
...
const dropFinance = 
`<div class="sell__action">
                        <ul class="sell__list">
                            <li class="sell__item"> <button>Pagamentos</button></li>
                            <li class="sell__item"> <button>Comissões</button></li>
                            <li class="sell__item"> <button>Bunus de performace</button></li>
                        </ul>
                    </div>`
...
const dropConfig = 
 `<div class="sell__action">
                        <ul class="sell__list">
                            <li class="sell__item"> <button>Membros</button></li>
                            <li class="sell__item"> <button>Gerais</button></li>
                            <li class="sell__item"> <button>Customizaveis</button></li>
                            <li class="sell__item"> <button>White Label</button></li>
                        </ul>
                    </div>`
...
const dropTools =     
                `<div class="sell__action">
                        <ul class="sell__list">
                            <li class="sell__item"> <button>Hubly</button></li>
                            <li class="sell__item"> <button>Retalier Form</button></li>
                            <li class="sell__item"> <button>Retalier Cluester</button></li>
                            <li class="sell__item"> <button>Retalier Invoice</button></li>
                        </ul>
                    </div>`
...
const DropdownList = () => {
    const drop = ['', dropSell, '', dropFinance, dropConfig, dropTools]
    return drop
}
...
const iconList = () => {
    const icons = [iconResume, iconSale, iconOrders, iconFinance, iconConfig, iconTools]
   return icons
}
...
const menuItems =  data => {
    ...
    menuDropdown = DropdownList()
    menuIcons = iconList()

    const menuGroupListLi = document.querySelector('[data-menuNav]')
    const menuHtmlArray = data.menuTree.map((menu, index) => {
      ...
      return `   
                <li class="nav__item"  data-dropdown>
                    ...
                    <button class="nav__link" data-dropdown-button>
                        ${menuIcons[index]}
                        ${menu.name}
                        ${ menu.hasChildren ? arrow : ''}
                    </button>
                ${menu.hasChildren ? (menuDropdown[index]) : ''}
            </li>
      `
      
    })
    
    const navHtml = menuHtmlArray.join(" ")
    menuGroupListLi.insertAdjacentHTML('beforeend', navHtml)
}
